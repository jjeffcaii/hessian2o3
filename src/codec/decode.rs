    ...
    R: io::Read + Sized,
{
    ...
    pub(crate) fn read_class_ref(&mut self) -> Result<(Cachestr, Fields)> {
        #[inline]
        fn seek_class_ref(ctx: Option<&Context>, i: usize) -> Option<(Cachestr, Fields)> {
            if let Some(c) = ctx {
                return c.nth(i);
            }
            None
        ...
        match self.peek()? {
            Header::BeginClassReference(i) => {
                ...
                let (class, fields) = seek_class_ref(self.ctx.as_ref(), class_ref as usize)
                    .ok_or_else(|| {
...
mod tests {
    ...
    fn test_get_value_bool() -> Result<()> {
        ...
        for next in [true, false] {
            ...
            let mut r = &b[..];
            let mut reader = Decoder::new(&mut r);

            let actual = reader.read_bool()?;
    ...
    #[test]
    fn test_get_value_string() -> Result<()> {
        init();
    ...
    #[test]
    fn test_get_value_binary() -> Result<()> {
        init();
    ...
    #[test]
    fn test_get_value_list() -> Result<()> {
        init();

        let class = "java.util.LinkedList";
        let expect = vec!["foo", "bar", "qux"];
        ...
        assert!(actual_class.is_some_and(|v| class == v.as_ref()));
    ...
    fn test_get_value_map() -> Result<()> {
        ...
        for next in [
            ...
            let b = hex::decode(next)?;

            let mut r = &b[..];
            let mut reader = Decoder::new(&mut r);

            assert!(reader.begin_map().is_ok());
        }
    ...
    fn test_object() -> Result<()> {
        init();
        {
            ...
            assert!(reader.read_class().is_ok());
        }
